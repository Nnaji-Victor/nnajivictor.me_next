...
const Featured = ({ featured }: FeaturedProps) => {
  ...
  return (
    <FeaturedContainer className="featured">
      <Heading content={"Featured"} className="animatedHeading">
        Featured Projects
      </Heading>
      {featured.map((featuredProject: FeaturedInterface, i: number) => {
        const {
          id,
          title,
          excerpt,
          slug,
          CaseStudiesGraphql: { featuredImage },
        } = featuredProject.node;
        return (
          <StyledFeatured
            className={`content-wrap slide slide--${i}`}
            id={`slide-${i}`}
            key={id}
          >
            <div className="col col--1">
              <div className="col__content">
                <h3 className="col__content-title">
                  {title.split(" ").map((text: string, i: number) => (
                    <span className="line__inner-mask" key={i}>
                      {SplitWord(text, "line__inner", i)}
                    </span>
                  ))}
                </h3>
